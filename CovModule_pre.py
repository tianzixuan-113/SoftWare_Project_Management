...
class StockDataset(Dataset):
    ...
    def __getitem__(self, index):
        return self.data[index//self.size][index%self.size:index%self.size+30].unsqueeze(0),self.data[index//self.size][index%self.size+31][:4]
...
class CovNN(nn.Module):
    def __init__(self):
        ...
        self.cov1=nn.Conv2d(1,10,kernel_size=3)
        self.cov2=nn.Conv2d(10,20,kernel_size=3)
        self.pooling=nn.MaxPool2d(2)
        self.l0=nn.Linear(240,128)
